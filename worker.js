...
Strict JSON only.`;
          ...
          quizData.answer = parsed.answer;
          quizData.explanation = parsed.explanation || null;
        }
      ...
      if (url.pathname === "/validate") {
        ...
        const current_question = body.question;
        const current_answer = body.correct_answer;
        ...
        if (!current_question || !current_answer) return new Response(JSON.stringify({ error: "question and correct_answer required for stateless validation" }), { status: 400, headers });
        ...
        const judgePrompt = `Question: "${current_question}"
Correct Answer: "${current_answer}" Validate the user's answer Mark as 'Correct' if the response is relevant to the question, otherwise 'Incorrect
...
Output JSON: {"correct": boolean, "explanation": "Short feedback in ${langName}"}`;
        ...
        const explanation = judgeResult.explanation || (isCorrect ? "Correct!" : `Incorrect. Answer: ${current_answer}`);
