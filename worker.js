  ...
  async fetch(request, env, ctx) {
    ...
    try {
      ...
      if (url.pathname === "/quizz") {
        ...
        let lang = body.lang?.trim().toLowerCase() || "en";

        let progress = await env.server.prepare("SELECT * FROM user_progress WHERE session_id = ?").bind(session_id).first();
        if (!progress) {
          await env.server.prepare("INSERT INTO user_progress (session_id, language, current_step, consecutive_correct) VALUES (?, ?, 1, 0)").bind(session_id, lang).run();
          progress = { language: lang, current_step: 1, consecutive_correct: 0 };
        } else if (lang !== progress.language) {
        ...
        if (randomType === "IDENTITY_IMAGE") {
          ...
          if (lang === "fr") questionText = "Qui est cette personne ?";
          else if (lang === "es") questionText = "¿Quién es esta persona?";
...
Format:
{
  ...
  "explanation": "Brief explanation"
      ...
      if (url.pathname === "/validate") {
        ...
        let lang = body.lang?.trim().toLowerCase() || progress.language || "en";
        const langName = lang === "fr" ? "French" : lang === "es" ? "Spanish" : "English";
