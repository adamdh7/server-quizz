    ...
    try {
      ...
      if (url.pathname === "/quizz") {
        ...
        if (randomType === "IDENTITY_IMAGE") {
          ...
          while (attempts < 5) {
            attempts++;
            const avoid = usedList ? `Exclude: ${usedList}.` : "";
            const personPrompt = `Generate the name of a globally famous person. ${avoid} Output ONLY the name.`;
          ...
          const systemPrompt = `Generate a ${langName} quiz question.
Difficulty: ${current_step_num}/10.
Type: ${randomType}.
...
Return strictly valid JSON with this format:
{
  ...
  "explanation": "explanation in ${langName}"
      ...
      if (url.pathname === "/validate") {
        ...
        const judgePrompt = `Analyze this quiz response.
Language: ${langName}
Question: "${current.question}"
Expected Answer: "${current.answer}"
User Input: "${user_answer}"

Determine if the User Input is correct based on the Question and Expected Answer. Use your judgment to validate.
...
{"correct": boolean, "explanation": "short feedback in ${langName}"}`;
        ...
        const judgeResp = await env.AI.run("@cf/meta/llama-3.1-8b-instruct", {
          messages: [
            { role: "system", content: "You are a JSON-only validator." },
