  ...
  async fetch(request, env, ctx) {
    ...
    try {
      ...
      if (url.pathname === "/quizz") {
        ...
        const baseQuestionTypes = ["MCQ", "TRUE_FALSE", "FILL_BLANK", "IDENTITY_IMAGE"];
        const extraTypes = [];
        if (current_step_num >= 7) extraTypes.push("TEXT_STYLE");
        const questionTypes = baseQuestionTypes.concat(extraTypes);
        ...
        if (randomType === "IDENTITY_IMAGE") {
          ...
          while (attempts < 5) {
            ...
            const personPrompt = `Give the full name of one globally famous historical figure or celebrity. ${avoid} Output ONLY the name.` ;

            const nameResp = await env.AI.run("@cf/meta/llama-3.1-8b-instruct", {
              messages: [
                { role: "system", content: "You are a concise name generator." },
          ...
          const imageInputs = {
            prompt: `Professional portrait of ${personName}, realistic, studio lighting, high detail`,
            num_steps: 4,
          };
          ...
          const shortPrompts = {
            MCQ: `Make 1 multiple-choice question in ${langName}. Difficulty ${current_step_num}. Return JSON with question, options (4 strings), answer, explanation.`,
            TRUE_FALSE: `Make 1 true/false question in ${langName}. Difficulty ${current_step_num}. Return JSON with question, options=null, answer ("True" or "False"), explanation.`,
            FILL_BLANK: `Make 1 fill-in-the-blank sentence in ${langName}. Difficulty ${current_step_num}. Provide JSON with question (with blank), options=null, answer (one word), explanation.`,
            TEXT_STYLE: `Create 1 short word-transform challenge in ${langName}. Provide JSON with question (instruction for user), prompt_word (transformed), answer (exact expected reply), options=null, explanation. Use a simple consistent transform (reverse or letter-swap).`
          };

          const sys = shortPrompts[randomType] || `Create 1 question in ${langName}. Difficulty ${current_step_num}. Return JSON with question, options or null, answer, explanation.`;
          ...
          if (randomType === "TEXT_STYLE") {
            if (parsed.prompt_word) quizData.prompt_word = parsed.prompt_word;
          }
        }
      ...
      if (url.pathname === "/validate") {
        ...
        let judgePrompt = "";
        if (current.q_type === "IDENTITY_IMAGE") {
          judgePrompt = `Role: Compare user answer to the expected answer. Question: "${current.question}" User Answer: "${user_answer}" Expected Answer: "${current.answer}" Output ONLY valid JSON: {"correct": boolean, "explanation": "Short feedback in ${langName}"}`;
        } else {
          judgePrompt = `Role: Compare user answer to the question. Question: "${current.question}" User Answer: "${user_answer}" Output ONLY valid JSON: {"correct": boolean, "explanation": "Short feedback in ${langName}"}`;
        }
        ...
        const threshold = Math.max(3, 7 - Math.floor(progress.current_step / 3));
        ...
        const response = {
          ...
          needed_for_next_level: Math.max(0, threshold - new_consec),
      ...
      if (url.pathname === "/step") {
        ...
        return new Response(JSON.stringify({
          ...
          needed_for_next_level: Math.max(0, threshold - progress.consecutive_correct)
        }), { headers });
      }
